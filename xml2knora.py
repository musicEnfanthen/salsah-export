...
class KnoraValue:
    ...
    def __init__(self, context: etree.iterparse, node: Tuple, type: str) -> None:
        self.resrefs = None
...
class KnoraProperty:
    ...
    def __init__(self, context: etree.iterparse, node: Tuple, type):
        ...
        while True:
            ...
            if event == 'start':
                if subnode.tag == type:
                    self.values.append(KnoraValue(context, subnode, type))
                else:
...
def do_sortorder(resources: List[KnoraResource]) -> List[KnoraResource]:
    ...
    cnt = 0
    notok_len = 9999999
    while len(resources) > 0 and cnt < 1000:
        for resource in resources:
            ...
            if len(resptrs) == 0:
                ...
                ok = True
                print('unique: ', resource.unique_id)
                pprint(resource.get_resptrs())
                print('..............')
        ...
        if not len(notok_resources) < notok_len:
            ...
            for r in notok_resources:
                print('unique: ', r.unique_id)
                pprint(r.get_resptrs())
                print('=============')
            exit(5)
...
def program(args) -> None:
    ...
    graph = con.get_ontology_graph(shortcode, args.ontoname)
    schema = con.create_schema(shortcode, args.ontoname)

    for resource in resources:
        #res_info = con.create_resource(resource.restype, resource.label, resource.get_propvals())
        #pprint(inst1_info)

        print('resclass=' + resource.restype)
        print('label=' + resource.label)
        pprint(resource.get_propvals())
